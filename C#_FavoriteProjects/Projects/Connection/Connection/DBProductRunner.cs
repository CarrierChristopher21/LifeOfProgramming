...
namespace Connection
{
    ...
    {
        /*  Information about DBProductRunner Class
            Now it gives you 3 choices to pick from. The first option asks you to pick a number between two numbers which would give you a file of that 
            specific number over a list of file data. You could also press 0 and follow that route which would give you an entire list 
            of the data instead of a single file's worth of data. If you go down the list of files then you also get the option to 
            ascend or descend the list from 1-77 or 77-1. The second option has you pick a number between 1-8 which represents 
            categoryID's so by picking this option the program filters and then outputs the files that have that number as its data entry.
            The third option is to range the unit price which can range from 0 to as high as you want. Yet the highest number is somewhere
            in the 200's. With this 3rd option it will filter any numbers that range between the two numbers that you entered. Then it will
            display those files for you to look at!
        */
        ...
        public string ProductCategoryConverter(string answer, int anId)
        {
            ...
            {
                DBPrinter.PrintProduct(this.aDBController.GetProductCategory(answer));
            }
            else
                NotValid.NotValidFile(answer);
            return answer;
